...
def head_tail_padding(num, max_length):
    pad_index = 0
    head_padding_list, tail_padding_list = [], []
    for protein in num:
        ...
        if (len(protein) < max_length):
            ...
            tail_padding_list.append(protein_pad)
        else:
            protein_head = protein_pad[:max_length]
            protein_tail = protein_pad[-1*max_length:]
            head_padding_list.append(protein_head)
            tail_padding_list.append(protein_tail)
    return head_padding_list, tail_padding_list
...
def collate_fn(data): 
    ...
    head1, tail1 = head_tail_padding(data_num, max_length1)
    head2, tail2 = head_tail_padding(data_num, max_length2)
    head3, tail3 = head_tail_padding(data_num, max_length3)
    head1 = torch.LongTensor(head1)
    head2 = torch.LongTensor(head2)
    head3 = torch.LongTensor(head3)
    tail1 = torch.LongTensor(tail1)
    tail2 = torch.LongTensor(tail2)
    tail3 = torch.LongTensor(tail3)

    data_label_ten = torch.LongTensor(data_label_ten)

    return (head1, tail1, head2, tail2, head3, tail3), data_label_ten   
...
class Net_multi(nn.Module):
    def __init__(self, multi, vocab_size, embedding_num, hidden_dim, num_layers, biFlag, dropout=0.2):  
        ...
        self.linear = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear((2*multi*self.bi_num*self.hidden_dim), 256),  #128
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, input1, input2, input3, input4, input5, input6):
        ...
        output4 = self.embedding(input4)
        output4, (ht,ct) = self.lstm(output4)
        output4 = output4.permute(0, 2, 1)
        output4 = self.globalmaxpool(output4).squeeze()
        
        output5 = self.embedding(input5)
        output5, (ht,ct) = self.lstm(output5)
        output5 = output5.permute(0, 2, 1)
        output5 = self.globalmaxpool(output5).squeeze()

        output6 = self.embedding(input6)
        output6, (ht,ct) = self.lstm(output6)
        output6 = output6.permute(0, 2, 1)
        output6 = self.globalmaxpool(output6).squeeze()
...
if __name__== '__main__':
    ...
    for time in range(len(pos_seed_list)):
        ...
        save_csv_path = './output_dir/Bilstm_MS_output/Bilstm_MS3_HeadTail_em1024_512_1_mydata_{}.csv'.format((time+1))
        ...
        for fold, (train_val_idx, test_idx) in enumerate(skf.split(X, Y)):       # train:val:test=7:1:2 /6:2:2
            ...
            model = Net_multi(3, len(vocab)+1, 1024, 512, 1, True)  # 512,256,1, hidden128表示256,128,1, em512:512,100,1 
            ...
            for epoch in range(1, epochs):
                ...
                for input, label in train_dataloader:
                    head1, tail1, head2, tail2, head3, tail3 = input
                    head1 = head1.to(device)
                    tail1 = tail1.to(device)
                    head2 = head2.to(device)
                    tail2 = tail2.to(device)
                    head3 = head3.to(device)
                    tail3 = tail3.to(device)
                    label = label.to(device)

                    logits1 = model(head1, tail1, head2, tail2, head3, tail3) 
